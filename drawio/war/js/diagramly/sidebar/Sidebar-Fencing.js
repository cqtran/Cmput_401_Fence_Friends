(function()
{
	Sidebar.prototype.addFencingPalette = function()
	{
		...
		[
			this.createVertexTemplateEntry(s2 + 'fence', w, h, '', 'Fence',
				null, null, this.getTagsForStencil(gn, 'fence', dt).join(' ')),

			this.createVertexTemplateEntry(s2 + 'gate', w, h, '', 'Gate', null,
				null, this.getTagsForStencil(gn, 'gate', dt).join(' ')),

			this.createVertexTemplateEntry(s2 + 'double_gate', w, h, '',
				'Double Gate', null, null,
				this.getTagsForStencil(gn, 'double_gate', dt).join(' ')),

			this.createVertexTemplateEntry(s2 + 'building', w, h, '',
				'Building', null, null,
				this.getTagsForStencil(gn, 'building', dt).join(' '))
